...
class VisionBinary:
    ...
    def __init__(self):
        ...
        self.resolution='10x'
        self.imagedir=os.path.join('/data/train_val_test',self.resolution)
        self.outdir='/data/train_val_test/output'
        #self.testPath=os.path.join(self.imagedir,'test')
        self.model_name=self.resolution+'_model'
        self.tr_name='train'
        self.val_name='val'
        self.arch='resnet101'
        self.img_sz=300
    ...
    def load_jpg_from_folder(self):
        ...
        tfms = get_transforms(flip_vert=True,max_rotate=self.rotate,max_warp=0.05,max_lighting = self.lighting)
    ...
    def train(self):
        ...
        learn = cnn_learner(data, self.model_dict[self.arch][0],
                            ...
                            callback_fns=[ShowGraph,
                                          ...
                                          partial(EarlyStoppingCallback, monitor='valid_loss', min_delta=0.001,patience=25)],
                            wd=0.1,
                            loss_func=torch.nn.CrossEntropyLoss(weight=w))
        #torch.nn.CrossEntropyLoss(weight=w))
        #loss_func=LabelSmoothingCrossEntropy(weight=w)).mixup().to_fp16()
...
class LabelSmoothingCrossEntropy(nn.Module):
    def __init__(self, ε: float = 0.3, reduction='mean'):
        super().__init__()
...
if __name__ == '__main__':
    ...
    #
    #c.apply_test_vote()_morphology/blob/master/prostate_binary.p
